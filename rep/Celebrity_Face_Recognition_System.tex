...
\subsection{Pipeline Visualization and Practical Validation}

This part of the report is particularly important as it demonstrates the practical
implementation and real-world behavior of the proposed face recognition system.
While quantitative results provide objective performance measurements, visual inspection
through the graphical user interface enables qualitative validation of the entire pipeline.

The graphical application makes it possible to observe the interaction between the different
processing stages in a transparent and interpretable manner. Each step of the pipeline,
including person detection (YOLO), face extraction (RetinaFace), and identity prediction
(VGG-Face + SVM), is executed sequentially and visualized in real time.

Bounding boxes corresponding to detected persons and extracted faces are displayed directly
on the input images, along with predicted identities and associated confidence scores.
This visualization allows verification that detections are spatially consistent, that faces
are correctly localized, and that identity predictions are coherent with visual appearance.

Figure~\ref{fig:app_gui} illustrates two representative use cases of the application.
The first example shows a multi-person scenario, where multiple individuals are detected
and correctly identified within the same image.
The second example presents a single-face recognition case, highlighting the confidence
score associated with the predicted identity.

These visual results confirm that the system is not only theoretically correct but also
fully operational and usable in practice. They provide an essential qualitative complement
to quantitative evaluation by revealing system behavior under realistic conditions,
including challenging backgrounds, varying poses, and multiple subjects.

Overall, the application interface plays a crucial role in validating the robustness,
interpretability, and practical applicability of the proposed face recognition pipeline.
\begin{figure}[H]
\centering
\includegraphics[width=0.48\textwidth]{app_demo_multi.png}
\hfill
\includegraphics[width=0.48\textwidth]{app_demo_single.png}
\caption{Graphical user interface of the face recognition application.
Left: multi-person detection and identification using YOLO and RetinaFace.
Right: single-face recognition with confidence score displayed.}
\label{fig:app_gui}
\end{figure}
...
\subsection{Command-Line Interface (CLI) Execution}

In addition to the graphical user interface, the face recognition system also provides
a command-line interface (CLI) mode. This execution mode is particularly important for
automation, batch processing, and reproducibility of experiments.

The CLI allows users to process either a single image or an entire directory by specifying
the input path and selecting the desired face embedding model.
This mode follows exactly the same processing pipeline as the graphical application,
including person detection using YOLO, face extraction with RetinaFace, and identity
prediction based on deep facial embeddings and an SVM classifier.

During execution, detailed logs are displayed in the terminal, including information about
the selected model, processing stages, execution time, and prediction results.
This textual feedback provides full transparency on the internal behavior of the system
and facilitates debugging, performance analysis, and large-scale evaluation.

Figure~\ref{fig:cli_execution} illustrates an example of CLI execution, showing the sequential
processing of the pipeline and the final identity predictions with associated confidence
scores. This confirms that the proposed system can be deployed both interactively and in
fully automated environments, increasing its practical applicability.
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{cli_execution.png}
\caption{Example of command-line execution of the face recognition pipeline.
The output shows model selection, YOLO detection, RetinaFace extraction, and final
identity predictions with confidence scores.}
...
recognition performance.The 61.1\% accuracy reflects the performance of the full end-to-end pipeline on the complete
dataset, while the higher accuracy values reported in the model comparison correspond to a
controlled evaluation protocol designed to compare embedding models under identical conditions.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{model_accuracy_comparison.jpeg}
\caption{Recognition accuracy comparison between ArcFace, FaceNet, and VGG-Face models.}
\label{fig:model_accuracy}
\end{figure}
Figure~\ref{fig:model_accuracy} shows the recognition accuracy obtained with different
face embedding models under identical experimental conditions.
Among the evaluated architectures, VGG-Face achieves the highest accuracy (0.88),
outperforming ArcFace (0.83) and FaceNet (0.81).

This result indicates that, despite its higher embedding dimensionality and computational
cost, VGG-Face provides stronger discriminative power in this experimental setup.
The larger representation capacity appears to better capture identity-specific facial
features, particularly in the presence of challenging conditions such as blur, pose
...
In addition, the comparative accuracy analysis highlights that VGG-Face outperforms both
ArcFace and FaceNet on the considered dataset. While ArcFace offers a good compromise
between precision and embedding compactness, VGG-Face achieves superior recognition
accuracy, at the cost of higher dimensionality.
This result suggests that, in closed-set celebrity identification scenarios, discriminative
capacity may outweigh memory efficiency when computational resources are not a limiting
factor.

The comparison between different embedding models confirms that no single architecture
is universally optimal. However, in the context of this project, VGG-Face consistently
achieves the highest recognition accuracy.
This suggests that accuracy-oriented applications benefit from higher-capacity embeddings,
while more compact models such as ArcFace or FaceNet remain relevant when computational
efficiency or memory constraints are critical.
...
A comparative analysis of multiple face embedding models shows that VGG-Face achieves the
highest recognition accuracy in this experimental setup, confirming its strong discriminative
capabilities when data quality is properly controlled.

The comparative evaluation of multiple embedding models further demonstrates that
VGG-Face provides the best recognition accuracy on this dataset, reinforcing the importance
of selecting feature representations adapted to the target application rather than relying
solely on model compactness.


The application successfully supports both single-image analysis and large-scale batch
processing, enabling comprehensive evaluation and practical usability. The availability of
both graphical and command-line execution modes further strengthens the practical relevance
of the proposed system, allowing intuitive interactive use as well as reproducible and
scalable experimentation through automated batch processing.
